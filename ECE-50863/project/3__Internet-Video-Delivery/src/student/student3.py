...
def BBA2(client_message: ClientMessage):
    ...
    reservoir       = sum([1 - size[0] / chunk_size[0] for size in client_message.upcoming_quality_bitrates[:int(max_buffer)]])
    ...
    if STARTUP_STAGE:
        ...
        selected_quality = min(2, prev_Q+1) if delta_B > 0.875 else prev_Q
        ...
        elif cur_buffer >= max_buffer * 0.9: selected_quality = rate_to_quality(chunk_size, R_max)
        else: selected_quality = rate_to_quality(chunk_size, chunk_map(chunk_size, cur_buffer, R_minus, R_plus, prev_R))
