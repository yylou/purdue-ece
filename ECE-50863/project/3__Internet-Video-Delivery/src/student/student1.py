...
def RobustMPC(client_message: ClientMessage):
    # QoE Initialization
    look_ahead_size = 5
    permutation = [_ for _ in itertools.product([q for q in range(1, client_message.quality_levels + 1)], repeat=look_ahead_size)]

    QoE     = dict()
    for _ in permutation: QoE[_] = 0

    # Quality + Quality Variation
    for key in QoE:
        QoE[key] = sum(key) * client_message.quality_coefficient
        variation = 0
        for i in range(1, len(key)):
            variation += abs(key[i] - key[i-1])
        QoE[key] -= client_message.variation_coefficient * variation

    cur_buffer = client_message.buffer_seconds_until_empty
    throughput = client_message.previous_throughput if client_message.previous_throughput != 0 else 1.5
    ...
    for key in QoE:
        ...
        QoE[key] -= client_message.rebuffering_coefficient * rebuffer_time

    Q = max(QoE, key= lambda x: QoE[x])[0] - 1

    # Store Data
    prev_quality.append(Q)
    prev_throughput.append(client_message.previous_throughput)
    prev_chunk.append(client_message.quality_bitrates)
...
def student_entrypoint(client_message: ClientMessage):
    ...
    return RobustMPC(client_message)