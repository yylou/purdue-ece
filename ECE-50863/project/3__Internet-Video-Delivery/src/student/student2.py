...
def RobustMPC_mod(client_message: ClientMessage):
    # QoE Initialization
    look_ahead_size = 9
    ...
    for key in QoE:
        ...
        for i in range(0, len(key)):
            if i == 0: 
                if prev_quality: variation += abs(key[i] - prev_quality[-1])
                else: variation += 0
            else: variation += abs(key[i] - key[i-1])
        
        QoE[key].append(client_message.quality_coefficient   * QoE_score)
    ...
    for key in QoE:
        ...
        QoE[key].append(2 * tmp_buffer)

    Q = max(QoE, key= lambda x: sum(QoE[x]))[0] - 1

    # Store Data
    prev_quality.append(Q)
    prev_throughput.append(throughput)
...
def student_entrypoint(client_message: ClientMessage):
    ...
    """
    TODO    (1) Consider amount of buffer for prediction
            (2) Extend look-ahead window size
            (3) Adjust weights for the look-ahead window
            (3) Consider the historical record of selected quality, throughput variation

            (a) Update QoE Initialization
    """

    return RobustMPC_mod(client_message)