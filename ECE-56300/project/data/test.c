...
int flag[2];
int global_count = 0;
char texttmp [15][100000*WORD_NUM];
...
// Reader function 
void reader(const char* filename, int i) 
{
...
int main(int argc, char** argv) {
    ...
    #pragma omp parallel for private(global_count)
    for(i = 0; i < argc - 1; i++) {
    //printf("name: %s \n", argv[i+1]);
        reader(argv[i+1], i);
    }
    ...
    for(i = 0; i < argc - 1; i++){
        strcat(text, texttmp[i]);
        strcat(text, " ");
    }

    // double readertime = omp_get_wtime();
    // printf("reader time(second): %f\n", readertime-begin);
    ...
    if (pid == 0) {
        ...
        while(i < numP) {
            ...
            MPI_Irecv(&rec[i], 1000000, Particletype, i, i, MPI_COMM_WORLD, request+i-1);
            i++;
        }
